...
%%For a full description of the input variables, see
%%simulate_data_basic.m; this function has three additional inputs.
%%half_window = physical distance on either side of the midpoint over
%%which local recombination rate is tabulated (e.g., 5000; note this
%%isn't currently used however).
%%f_factor = ratio of gene conversion rate to rate of crossing-over (e.g., 3.75)
%%half_tract = half the length of gene conversion tracts (e.g., 50)


function [dnum_sim,dden_sim,het_rate_sim,sum_phys,sum_cM] = simulate_data_gene_conversion(half_window,f_factor,half_tract,print,inter_var,intra_var,mu,N,sim_history,mincount,maxcount,ms_path,out_dir,out_name,chr,phys,map,block_starts,blocks_left,blocks_right,sim_reps,sim_map)
...
%%For PSMC inference, only use one copy of each chromosome from among all
%%simulated samples, chosen at random
...
for i = 1:size(blocks_left,1) %loop over regions
    len = phys(blocks_right(i))-phys(blocks_left(i)); %physical length of
                                                      %region
    num_spots = blocks_right(i)-blocks_left(i); %number of "hotspots,"
                                                %i.e., changes in
                                                %recombination rate
                                                %within region
    ...
    command = sprintf(['%s 2 %d -seeds %d %d %d -' ...
                       't %f -r %f %d -c %f %d %s'],ms_path,sim_reps, ...
                      seeds(1),seeds(2),seeds(3),theta,rho,len,f_factor, ...
                      half_tract,sim_history);
    crossover_command = sprintf('-v %d',num_spots);
    for s = 1:num_spots
        ...
        crossover_command = sprintf('%s %s',crossover_command,spot);
    end
    command = sprintf('%s %s',command,crossover_command);
    command = strcat(command,' | egrep ''positions:|segsites: 0''');

    %call msHOT    
    [~, result] = system(command);
    ends = [0 strfind(result,char(10))]; %keep track of the line breaks
                                         %in the msHOT output, which mark
                                         %the boundaries between the output for each genome
    
    %bin setup
    midp_phys = 50000+phys(block_starts(i));
    jack_ind = 1:23;
    jack_ind(chr(block_starts(i))) = []; %for the jackknife
    midp_cM = interp1q(phys(blocks_left(i):blocks_right(i)), ...
                       map(blocks_left(i):blocks_right(i)),midp_phys);
    left_cM = interp1q(phys(blocks_left(i):blocks_right(i)), ...
                       map(blocks_left(i):blocks_right(i)),midp_phys-half_window);
    right_cM = interp1q(phys(blocks_left(i):blocks_right(i)),map(blocks_left(i):blocks_right(i)),midp_phys+half_window);
    jack_den = zeros(1,23);
    ...
    for j = 1:size(ends,2)-1 %loop over genomes
     ...
     %%Write psmcfa output for selected genomes
        ...
        seg_phys = datarow*len+phys(blocks_left(i)); %physical positions
                                                     %of het sites
        ...
        if cold_pos > 0 && cold_neg > 0 && coldcount >= mincount && coldcount <= maxcount
            ...
            sum_phys = sum_phys + 10000;
            sum_cM = sum_cM + right_cM - left_cM;
...
save(var_file,'dnum_sim','dden_sim','het_rate_sim','sum_phys','sum_cM');
end
if print >= 1
for i = print:23
    fclose(fid_psmc(i));
