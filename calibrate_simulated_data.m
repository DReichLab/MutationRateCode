...
%%This function generates calibration data to match simulated test data.
%%Many inputs are similar to simulate_data_basic.m, but some differ:
%%seed_base = a small integer (between approximately 0 and 400) used for
%%randomization
%%chromosome = number of chromosome to omit, for jackknife purposes; if
%%desired to make calibration for all chromosomes, enter 0 or 23
%%cal_values = values of mu for which to make calibration curves, in
%%units of 10^-8 per base per generation (e.g., [1 2 4])
%%dnum_sim, dden_sim, het_rate_sim = output of simulate_data_basic.m
%%theta_base = per-base value of theta for msHOT (e.g., 20000/30000000)
%%history = inferred pop size history for msHOT
%%chr, phys, map = base map variables
%%cal_genomes = number of genomes' worth of data to generate per
%%calibration curve
%%cal_chr, cal_phys, cal_map = map variables to use for calibration
%%(usually the cal_map will be a perturbed version of the base map)
%%cal_blocks_left, cal_blocks_right = regions for calibration; typically
%%the same as blocks_left and blocks_right


function [mu_cal,cal,het_rate_cal] = calibrate_simulated_data(seed_base,chromosome,cal_values,mincount,maxcount,ms_path,dnum_sim,dden_sim,het_rate_sim,theta_base,history,chr,phys,map,block_starts,blocks_left,blocks_right,cal_genomes,cal_chr,cal_phys,cal_map,cal_blocks_left,cal_blocks_right)
...
if chromosome ~= chr(block_starts(i)) %for calibrating jackknife replicates
    ...
    for j = 1:cal_genomes
        ...
        if seed_base > 0
            rng(500*25000*seed_base+100*25000*z+100*i+j);
        end         
        ...
        tot_sites_cal = tot_sites_cal + len;
