...
%%simulate_data_basic.m; this function has two additional inputs,
%%seq_error, which specifies the rate per base at which false het sites are added
%%to the simulated data (e.g., 10^-5), and half_window, which specifies
%%the physical distance on either side of the midpoint over which errors
%%are counted (e.g., 15000).


function [dnum_sim,dden_sim,het_rate_sim,errors,tot_sites] = simulate_data_errors(half_window,print,inter_var,intra_var,mu,N,sim_history,mincount,maxcount,ms_path,out_dir,out_name,seq_error,chr,phys,map,block_starts,blocks_left,blocks_right,sim_reps,sim_map)
...
tot_segsites_sim = 0;
tot_sites_sim = 0;
errors = 0; %for keeping track of errors
tot_sites = 0; %denominator for error count
...
for i = 1:size(blocks_left,1)
    ...
    command = sprintf('/home/ml275/msHOTnew/msHOT 2 %d -seeds %d %d %d -t %f -r %f %d %s -v %d',sim_reps,seeds(1),seeds(2),seeds(3),theta,rho,len,sim_history,num_spots);
    for s = 1:num_spots
    ...
    for j = 1:size(ends,2)-1 %loop over genomes
     ...
     if intra_var == -1    
      ...
      if seq_error > 0   
        ...
        datarow_clean = datarow;
        num_errors = poissrnd(len*seq_error);
        error_positions = rand(num_errors,1);
        datarow = [datarow_clean; error_positions];
     end      
     ...
     %%Write psmcfa output for selected genomes
        ...
        seg_phys = datarow*len+phys(blocks_left(i)); %physical positions
                                                     %of het sites
        seg_phys_clean = datarow_clean*len+phys(blocks_left(i)); %true
                                                                 %hets only
        ...
        if cold_pos > 0 && cold_neg > 0 && coldcount >= mincount && coldcount <= maxcount
            ...
            errors = errors + sum(abs(seg_phys-midp_phys) < half_window) - sum(abs(seg_phys_clean-midp_phys) < half_window);
            tot_sites = tot_sites + 2*half_window;
...
save(var_file,'dnum_sim','dden_sim','het_rate_sim','errors','tot_sites');
end
if print >= 1
for i = print:23
    fclose(fid_psmc(i));
